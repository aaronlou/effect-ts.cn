...
status: published
upstreamPath: v3/sink/introduction.mdx
...
在 Stream 处理中，`Sink` 是一种用于消费 `Stream` 所生成元素的结构。
...
- `Chunk<number>`：Sink 处理完元素后产出的最终结果（在本例中，是一个由数字组成的 [Chunk](/docs/v3/data-types/chunk/)）。
