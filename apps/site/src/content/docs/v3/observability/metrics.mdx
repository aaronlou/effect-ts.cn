...
status: published
upstreamPath: v3/observability/metrics.mdx
...
Effect Metrics API 还允许你为 Summary 配置**误差范围（error margin）**。这个范围会为分位数引入一个可接受值的区间，从而提高结果的准确性。
...
你可以使用 `Metric.tagged` 函数为单个 metric 打标签。
这让你可以为单个 metric 添加特定的标签，提供详细的上下文，而无需全局应用标签。
...
如果你只想在特定的 [scope](/docs/v3/resource-management/scope/) 内应用标签，可以使用 `Effect.tagMetricsScoped`。这会把标签的应用限制在该 scope 内的 metric 上，从而实现更精确的标签控制。
