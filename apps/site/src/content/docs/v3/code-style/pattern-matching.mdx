...
status: published
upstreamPath: v3/code-style/pattern-matching.mdx
...
`Match.option` 会把匹配结果包装进一个 [Option](/docs/v3/data-types/option/)。如果找到匹配，它会返回 `Some(value)`；否则返回 `None`。
...
`Match.either` 方法会把结果包装进一个 [Either](/docs/v3/data-types/either/)，提供一种结构化的方式来区分匹配与未匹配的情况。如果找到匹配，它会返回 `Right(value)`；否则返回 `Left(no match)`。
