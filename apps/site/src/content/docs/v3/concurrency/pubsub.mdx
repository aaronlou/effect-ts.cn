...
status: published
upstreamPath: v3/concurrency/pubsub.mdx
...
与 [Queue](/docs/v3/concurrency/queue/) 不同——在 Queue 中每个值只会投递给一个消费者——`PubSub` 会把每条已发布的消息广播给所有订阅者。因此，在需要消息广播而非负载分发的场景中，`PubSub` 是理想之选。
...
| `PubSub.subscribe` | 创建一个 scoped effect，用于订阅该 `PubSub`，并在作用域结束时自动取消订阅。订阅者通过 [Dequeue](/docs/v3/concurrency/queue/#dequeue) 接收消息，Dequeue 中保存着已发布的消息。 |
...
`PubSub` 的操作符与 [Queue](/docs/v3/concurrency/queue/) 类似，主要区别在于用 `PubSub.publish` 和 `PubSub.subscribe` 代替了 `Queue.offer` 和 `Queue.take`。如果你已经熟悉 `Queue` 的用法，那么 `PubSub` 对你来说会很容易上手。
