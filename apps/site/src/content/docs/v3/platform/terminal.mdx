---
title: Terminal
description: 与标准输入和标准输出交互，读取用户输入并在终端上显示消息。
status: published
upstreamPath: v3/platform/terminal.mdx
...
该模块只提供一个 `Terminal` [tag](/docs/v3/requirements-management/services/)，它是从标准输入读取、向标准输出写入的入口。
