...
status: published
upstreamPath: v4/observability/logging.mdx
...
| `fiber` | 执行该程序的 [fiber](/docs/v4/concurrency/fibers/) 的标识符。 |
...
为了提供更多上下文，你还可以在日志中包含一个或多个 [Cause](/docs/v4/data-types/cause/) 实例，
...
默认情况下，`DEBUG` 消息**不会被展示**。要为某个 effect 启用它们，请提供值为 `"Debug"` 的 `References.MinimumLogLevel` 上下文引用。
...
若要从[配置](/docs/v4/configuration/)中加载日志级别并应用到你的程序，请把配置值映射为一个用于 `References.MinimumLogLevel` 的 Layer。
...
<Aside type="tip" title="Using ConfigProvider for Testing">
  ...
  示例见[加载内存中的对象](/docs/v4/configuration/#loading-an-in-memory-object)。
</Aside>
...
每个 logger 都以两种形式提供：logger 本身，以及一个使用该 logger 并把输出发送到 `Console` [默认服务](/docs/v4/requirements-management/default-services/) 的 layer。例如，`structuredLogger` logger 以详细的对象格式生成日志，而 `structured` layer 使用同一个 logger，并把输出写入 `Console` 服务。
...
| `level`       | 表示日志级别标签的字符串（例如 "INFO" 或 "DEBUG"）。                                                                                                                      |
