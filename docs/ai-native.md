...
- **形象**：一只**像素风**的猫，名叫**小效**，戴着赛博护目镜。选猫不是随手挑的：Effect 的并发原语叫
  **Fiber**（轻量、可成千上万），而"猫有九条命"是最容易被中文读者记住的类比；项圈挂着 Effect 徽记。
  像素 + 护目镜 + 霓虹天线，是「终端 / 游戏 / 科幻」这一族视觉语言里辨识度最高的一种，
  而且像素网格在小尺寸（52px 常驻、30px 面板头部）天生不糊 —— 这是第一版矢量猫的实际教训。
...
- **显眼但安静**：整像素跳动悬浮（2.4s，`steps(2)`，不是平滑浮动 —— 像素精灵的悬浮感来自"跳格"）
  + 每 6.8s 一次眨眼（眼睛像素整块消失，`steps(1)`，不插值）+ 天线信号灯呼吸；每个会话**只**冒一次招呼气泡
...
- **实现**：`apps/site/src/components/Mascot.astro` —— 用 **20×20 字符网格**画（`PIXELS` 里改一个
  字符就改一个像素，比手写贝塞尔路径可维护得多），配 `shape-rendering="crispEdges"` + 整数坐标；
  **轮廓是扫描出来的**（任何四邻有填充的空白格变成描边像素），不是手画的；
  横向同色像素**合并成一条 rect**，元素数从 400 降到约 60 —— 同一页要出现 4~5 次，这很关键。
  三套表情（happy / alert / curious）都在 DOM 里由 CSS 切换，因为 AgentDock 是按 `data-state` 切表情的。
  + `AgentDock.astro`（状态与交互）。
