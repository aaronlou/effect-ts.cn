...
### 3.9 成本闸门：公开发布后怎么不被打爆

公开站点最大的**财务**风险不是被攻击，而是"被正常地大量使用"：一次提问在开着模型时会触发
最多 4 次调用（追问改写 → 白话扩展 → 候选重排 → 答案润色），而每分钟限流只管突发、不管总量
（20 次/分 = 单来源每天最多 28,800 次）。

#### 实测成本（不是估算）

用线上真实提示词形态对 DeepSeek 实测：

| 调用 | prompt | completion | 合计 |
| --- | --- | --- | --- |
| 答案润色 | 303 | 190 | **493 token** |
| 候选重排 | 264 | 60 | **324 token** |

即**一次带模型的问答约 800~1300 token**（追问改写只在多轮时发生；白话扩展只在首次检索偏弱时发生）。

#### 四道闸门（从外到内）

| 闸门 | 默认值 | 作用 |
| --- | --- | --- |
| **每日 token 预算**（`LLM_DAILY_TOKEN_BUDGET`） | 2,000,000 | **硬止损**：达到即停止调用模型，问答降级为检索合成 |
| 单来源每日提问上限（`ASK_DAILY_LIMIT_PER_IP`） | 80 | 防止一个来源把全局预算吃光 |
| 单来源每分钟限流（`ASK_RATE_LIMIT_PER_MINUTE`） | 20 | 防突发 |
| 答案缓存 | 2000 条 / 24 小时 | 命中即**零 token**（文档问答重复率极高） |

**为什么闸门必须在 API 这一侧**：我们握着唯一通向 Key 的出口。平台侧的额度/告警只能事后
告诉你花了多少，而站点无人看管时被打量，损失是实时的。闸门放在自己的调用路径上，才能在
超预算的**那一刻**停止花钱。

**超预算不等于站点挂掉**：所有模型调用短路返回"不采用模型输出"，问答自动退回
extractive —— 引用、拒答、检索全都照常，只是不再有模型润色。宁可朴素，不可下线。

#### 怎么看花了多少

```bash
curl -s https://effect-ts.cn/api/knowledge/stats | python3 -m json.tool
```
...
- `remaining === -1` 表示**未设上限**；
- `exhausted: true` 时问答面板会显示「检索合成（今日模型额度已用完）」——
  如实告诉用户为什么没有模型润色，而不是让人以为站点坏了；
- 预算按 **UTC 日**重置（重置点必须可预期，运维要知道几点恢复）。

#### 还想更省

- **调小 `LLM_DAILY_TOKEN_BUDGET`**：例如 20 万 ≈ 每天 150~250 次带模型的问答；
  超出部分自动走 extractive，站点仍完全可用（引用与拒答都还在）。
...
> **403 会瞬时出现，不要据此判断配置错了。** 首次提交时（key 文件上线约 10 分钟后）拿到 403，
> 而同一份 payload 几分钟后原样重放就是 200 —— 是对方的 key 校验还没传播，或缓存了一次
> "文件不存在"。脚本因此对 403/429/5xx 做了退避重试（5s → 20s → 60s）。
...
一个站主专用的仪表盘：<https://effect-ts.cn/admin/>

**三道防线，缺一不可：**
1. **不进导航** —— 全站没有任何链接指向它，只能靠记住地址进入；
2. **noindex + 不进 sitemap** —— 搜索引擎不会收录；
3. **接口要口令** —— 页面本身是静态的，数据全在 `/api/admin/traffic` 后面，
   没配 `ADMIN_TOKEN` 时该接口一律 401。

> 所以要清楚：**地址不是秘密，数据才是。** 猜到 `/admin/` 只会得到一个空壳。

口令在服务器的 `.env` 里（`grep ADMIN_TOKEN .env`）。**不配 = 后台关闭**，
这是刻意的默认 —— 一个"忘了配就等于公开"的管理接口是最典型的翻车方式。
报表只输出 IP 的**哈希前缀**，不含原始 IP（与 `/privacy` 的说法一致）。

#### 数据从哪来

**宿主 Caddy 日志**（compose 把 `/var/log/caddy` 只读挂进 api 容器）。
站点容器的 stdout 日志走 docker json-file 驱动，**容器一重建就清零**（每次部署都会），
只有宿主那份是持久的。api 容器以 root 运行，所以能读 `-rw------- caddy:caddy`。

**不落库**：日志本身就是数据库，按需读取 + 60 秒缓存。这样避开了采集服务、
offset 游标、轮转检测、去重一整套复杂度。

#### 口径（看数字前先看这段）

访客分**四类且互斥**：人类 / 收录型爬虫 / 漏洞扫描 / 探针，加起来等于总请求数。
混成笼统的"机器人"会让"站点变好了"和"被扫了"看起来一模一样。

★ **看「真实读者」而不是「独立访客」**：UA 分不出扫描器 —— 日志里大量请求伪装成
`Chrome/131`，只请求一条路径就走。真浏览器打开页面后**必然会拉 `/_astro/*.css|js`**，
所以「加载过静态资源的访客」才接近"有多少人在读"。
实测某 24 小时：独立访客 42，真实读者 **12**。

#### 踩过的坑：不要只看 referer 的域名就下结论

上线当天核对数据时发现，那 22 次 `www.google.com` 来源**全部来自同一个扫描器**：
UA 是 `Mozlila/5.0`（**Mozilla 的经典拼错**），请求 `//images/cache.php` 这类 PHP 漏洞路径，
referer 是伪造的。另一批 `www.baidu.com` 来源的 UA 是 `redroid12_arm64`
（Android 跑在容器里的自动化环境）。

**教训：referer 是客户端可控的，不能当作证据。** 判断一个来源是否真实，
必须同时看**请求路径**与 **UA** —— 真人会落在内容页、会拉静态资源；
扫描器只会打一条奇怪的路径。
