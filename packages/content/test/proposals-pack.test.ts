...
describe("proposals:pack", () => {
  ...
  it("--force 也不能把已落地页面打成提案（force 只覆盖同名文件）", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "ecn-pack-force-"))
    ...
    await writeFile(
      path.join(draftsDir, "v4__landed.mdx"),
      `---\ntitle: 示例\nstatus: reviewing\nupstreamPath: v4/landed.mdx\nupstreamCommit: bf4625446a02894046b6937a317dde2cde115fe7\ntranslators: [t]\nreviewers: []\n---\n\n正文\n`,
      "utf8"
    )
    const result = await packProposals({ draftsDir, outDir, docsDir, agent: "test", promptVersion: "translate-v1", force: true })
    expect(result.packed).toEqual([])
    expect(result.skipped.map((item) => item.id)).toEqual(["translation-v4-landed"])
  })
